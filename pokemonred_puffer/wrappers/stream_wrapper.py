...
import logging
from pokemonred_puffer.data import poke_dict
...
class StreamWrapper(gym.Wrapper):
    ...
    def step(self, action):
        ...
        self.stream_metadata = {
            "user": self.user,
            "env_id": f'\n{self.env.unwrapped.env_id} \n "levels": {self.env.unwrapped.levels}\n "lead_pokemon": {poke_dict[self.env.unwrapped.read_party()[0]["name"]]}\n "HMs in party": {self.env.unwrapped.moves_obtained}\n "badges": {self.env.unwrapped.get_badges()}\n \n'
        }
        print(f'{poke_dict[self.env.unwrapped.read_party()[0]["name"]]}\n')
        self.stream_metadata["color"] = "#FF8C00"
