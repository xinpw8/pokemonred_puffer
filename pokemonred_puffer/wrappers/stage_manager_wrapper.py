...
class StageManagerWrapper(ObsWrapper):
    def __init__(self, env: RedGymEnv, env_config: pufferlib.namespace):
        ...
        self.early_stopping = self.env.early_stopping # if 'early_stop' in self.env else None
        self.early_stopping_min_reward = self.env.early_stopping_min_reward if not None else 2.0
        self.save_video = self.env.save_video
        self.fast_video = self.env.fast_video
        self.video_interval = 256 * self.act_freq
        self.downsample_factor = 2
        self.frame_stacks = 3
        self.explore_weight = self.env.unwrapped.explore_weight if self.env.unwrapped.explore_weight is not None else 1
        self.use_screen_explore = self.env.unwrapped.use_screen_explore if self.env.unwrapped.use_screen_explore is not None else True
        self.randomize_first_ep_split_cnt = self.env.unwrapped.randomize_first_ep_split_cnt if self.env.unwrapped.randomize_first_ep_split_cnt is not None else 0
        # self.similar_frame_dist = self.env.unwrapped.sim_frame_dist
        self.reward_scale = self.env.unwrapped.reward_scale if self.env.unwrapped.reward_scale is not None else 1
        self.extra_buttons = self.env.unwrapped.extra_buttons if self.env.unwrapped.extra_buttons is not None else False
        self.noop_button = self.env.unwrapped.noop_button if self.env.unwrapped.noop_button is not None else False
        self.swap_button = self.env.unwrapped.swap_button if self.env.unwrapped.swap_button is not None else True
        self.restricted_start_menu = self.env.unwrapped.restricted_start_menu if self.env.unwrapped.restricted_start_menu is not None else False
        ...
        self.enable_stage_manager = self.env.unwrapped.enable_stage_manager if self.env.unwrapped.enable_stage_manager is not None else False
        self.enable_item_purchaser = self.env.unwrapped.enable_item_purchaser if self.env.unwrapped.enable_item_purchaser is not None else False
        self.auto_skip_anim = self.env.unwrapped.auto_skip_anim if self.env.unwrapped.auto_skip_anim is not None else False
        self.auto_skip_anim_frames = self.env.unwrapped.auto_skip_anim_frames if self.env.unwrapped.auto_skip_anim_frames is not None else 8
        self.env_id = self.env.unwrapped.env_id if self.env.unwrapped.env_id is not None else str(random.randint(1, 9999)).zfill(4)
        self.env_max_steps = self.env.unwrapped.env_config_max_steps if self.env.unwrapped.env_config_max_steps is not None else []
        self.total_envs = self.env.unwrapped.total_envs if self.env.unwrapped.total_envs is not None else 48
        self.level_manager_eval_mode = self.env.unwrapped.level_manager_eval_mode if self.env.unwrapped.level_manager_eval_mode is not None else False
    ...
    def scripted_routine_cut(self, action):
        ...
        if tile_id in [0, 7]:  # overworld, gym
            minimap_tree = self.env.unwrapped.get_minimap_obs()[1]
    ...
    def scripted_routine_surf(self, action):
        ...
        if facing_direction == 0:  # down
            tile_infront = self.env.unwrapped.bottom_left_screen_tiles[5, 4]
        elif facing_direction == 4:  # up
            tile_infront = self.env.unwrapped.bottom_left_screen_tiles[3, 4]
        elif facing_direction == 8:  # left
            tile_infront = self.env.unwrapped.bottom_left_screen_tiles[4, 3]
        elif facing_direction == 12:  # right
            tile_infront = self.env.unwrapped.bottom_left_screen_tiles[4, 5]
        # tile_infront = self.read_ram_m(RAM.wTileInFrontOfPlayer)
        # no_collision = True
        use_surf_now = False
        if tile_infront in [0x14, 0x32, 0x48]:
            ...
            if tile_id == 17:
                ...
                tile_standingon = self.env.unwrapped.bottom_left_screen_tiles[4, 4]
    ...
    def create_exploration_memory(self):
        w = self.env.unwrapped.output_shape[1]
        h = self.env.unwrapped.memory_height
        
        def make_reward_channel(r_val):
            col_steps = self.env.unwrapped.col_steps
