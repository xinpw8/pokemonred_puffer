...
class MultiConvolutionalPolicy(nn.Module):
    def __init__(
        ...
        self.encode_linear = nn.Sequential(
            nn.Linear(64 * 18 * 20, hidden_size),  # Adjusted based on PyBoy resolution
            nn.ReLU(),
        )
        ...
        if self.use_global_map:
            self.global_map_network = nn.Sequential(
                ...
                nn.ReLU(),
                nn.Flatten(),
                nn.Linear(64 * 18 * 20, 480),  # Adjusted based on PyBoy resolution
                nn.ReLU(),
            )
        ...
        self.minimap_cnn_linear = nn.Sequential(
            nn.Linear(minimap_cnn_output_dim, cnn_output_dim), nn.ReLU()
        )
