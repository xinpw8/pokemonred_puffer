...
class BaselineRewardEnv(RedGymEnv):
    ...
    def get_game_state_reward(self):
        ...
        return {
            ...
            "taught_cut": 4
            * int(self.check_if_party_has_hm(0x0F))
    ...
    def get_all_events_reward(self):
        c, r, map_n = ram_map.position(self.pyboy)
        if map_n in [186, 187]:
            return max(
                sum(
                    [
...
class CutWithObjectRewardsEnv(BaselineRewardEnv):
    ...
    def get_exp_bonus(self):
        ...
        if map_n in [186, 187]:
            rew = 0
        elif map_n in self.poketower_maps and int(ram_map.read_bit(self.pyboy, 0xD838, 7)) == 0:
            rew = 0
