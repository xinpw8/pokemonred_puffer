...
class RewardWrapper(RedGymEnv):
    def __init__(self, env_config: pufferlib.namespace, reward_config: pufferlib.namespace):
        ...
        self.reward_weights = reward_config if reward_config else {
            ...
            "bag_menu": 0.1,
            "healing_reward": 1.0,
            "opponent_level": 0.0006,
        }

    def get_game_state_reward(self):
        rewards = {
            "seen_pokemon_reward": self.reward_weights['seen_pokemon_reward'] * sum(self.seen_pokemon),
            "event": self.reward_weights['event'] * self.get_event_reward(),
            "exploration": self.reward_weights['exploration'] * self.get_exploration_reward(),
            "moves_obtained_reward": self.reward_weights['moves_obtained_reward'] * sum(self.moves_obtained),
            "level": self.reward_weights['level'] * self.get_level_reward(),
            "caught_pokemon_reward": self.reward_weights['caught_pokemon_reward'] * sum(self.caught_pokemon),
            "badges": self.reward_weights['badges'] * self.get_badges_reward(),
            "hm_count_reward": self.reward_weights['hm_count_reward'] * self.get_hm_reward(),
            "bill_saved_reward": self.reward_weights['bill_saved_reward'] * self.get_bill_reward(),
            "cut_tiles": self.reward_weights['cut_tiles'] * len(self.cut_tiles),
            "cut_coords": self.reward_weights['cut_coords'] * sum(self.cut_coords.values()),
            "used_cut": self.reward_weights['used_cut'] * 1.0,
            ...
            "healing_reward": self.reward_weights['healing_reward'] * self.get_heal_reward(),
            "opponent_level": self.reward_weights['opponent_level'] * self.get_opponent_level_reward(),
        }
        return rewards
